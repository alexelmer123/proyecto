...
/** @var array  $departamentos */ /** @var array $provincias */
/** @var array  $distritos */     /** @var array $ciudades */
...
$err  = static fn(string $k) => isset($errores[$k])
    ...
    : '';
$sel  = static fn(?int $current, $optId): string =>
            ...
            <label class="field">
                ...
                <select class="field__input" name="pais_id" data-cascade="departamento_id" data-cascade-url="<?= BASE_URL ?>/geografia/departamentos">
                    ...
                    <?php foreach ($paises as $p): ?>
                        <option value="<?= (int) $p['id'] ?>" <?= $sel($form['pais_id'] ?? null, $p['id']) ?>><?= $h($p['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
            <label class="field">
                ...
                <select class="field__input" name="departamento_id" data-cascade="provincia_id" data-cascade-url="<?= BASE_URL ?>/geografia/provincias" <?= empty($departamentos) ? 'disabled' : '' ?>>
                    ...
                    <?php foreach ($departamentos as $d): ?>
                        <option value="<?= (int) $d['id'] ?>" <?= $sel($form['departamento_id'] ?? null, $d['id']) ?>><?= $h($d['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
        </div>
            ...
            <label class="field">
                ...
                <select class="field__input" name="provincia_id" data-cascade="distrito_id" data-cascade-url="<?= BASE_URL ?>/geografia/distritos" <?= empty($provincias) ? 'disabled' : '' ?>>
                    ...
                    <?php foreach ($provincias as $p): ?>
                        <option value="<?= (int) $p['id'] ?>" <?= $sel($form['provincia_id'] ?? null, $p['id']) ?>><?= $h($p['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
            <label class="field">
                ...
                <select class="field__input" name="distrito_id" data-cascade="ciudad_id" data-cascade-url="<?= BASE_URL ?>/geografia/ciudades" <?= empty($distritos) ? 'disabled' : '' ?>>
                    ...
                    <?php foreach ($distritos as $d): ?>
                        <option value="<?= (int) $d['id'] ?>" <?= $sel($form['distrito_id'] ?? null, $d['id']) ?>><?= $h($d['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
            <label class="field">
                ...
                <select class="field__input" name="ciudad_id" <?= empty($ciudades) ? 'disabled' : '' ?>>
                    ...
                    <?php foreach ($ciudades as $c): ?>
                        <option value="<?= (int) $c['id'] ?>" <?= $sel($form['ciudad_id'] ?? null, $c['id']) ?>><?= $h($c['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
        </div>
