...
<header class="page-head">
    <div>
        ...
        <p class="page-head__caption mono">N° <?= $h($form['numero']) ?></p>
    </div>
    <a href="<?= BASE_URL ?>/pedido/index" class="btn btn--ghost">← Volver</a>
</header>

<form method="post" action="<?= BASE_URL ?>/pedido/crear" class="form form--card">
    <input type="hidden" name="numero" value="<?= $h($form['numero']) ?>">
    ...
    <div class="form__row form__row--3">
        ...
        <label class="field">
            ...
            <?= $err('pagado') ?>
            <small class="form__hint">El estado se calcula: pagado=total → pagado · 0&lt;pagado&lt;total → deuda · pagado=0 → pendiente.</small>
        </label>
    </div>
    ...
    <label class="field">
        ...
        <textarea class="field__input" name="observacion" rows="2"
                  placeholder="Detalles del pedido, número de OC, etc."><?= $h((string) ($form['observacion'] ?? '')) ?></textarea>
    </label>
